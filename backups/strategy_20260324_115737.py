...
def strategy(
    ...
    """Volatility & volume-adaptive LP strategy with enhanced mean reversion detection.

    Key changes from commit 69:
    - Increased mean_reversion_bonus max from 0.03 to 0.04
    - Adjusted volatility dampening to be less aggressive at moderate vol levels
    - Fine-tuned capital_fraction bounds: min 0.78, max 1.03
    - This allows more aggressive deployment when mean reversion signals are strong
    """
    ...
    if len(prices_so_far) >= 10:
        ...
        if price_std > 0:
            ...
            # Max bonus of 0.04 when deviation < 0.5 std (increased from 0.03)
    ...
    # --- Volatility & volume-adaptive capital deployment with refined curve ---
    base_capital = 0.975
    
    # Refined volatility dampening: less aggressive at moderate vol, more protection at spikes
    # Use a gentler curve that allows higher deployment when vol is 0.02-0.04
    vol_excess = max(0, recent_vol - 0.02)
    
    if vol_excess < 0.03:
        # Low excess vol: minimal dampening
        vol_adjustment = -0.08 * (vol_excess / 0.03)
    else:
        # Higher excess vol: stronger dampening with sqrt for smoother curve
        vol_adjustment = -0.08 - 0.12 * np.sqrt((vol_excess - 0.03) / 0.05)
    
    vol_adjustment = max(-0.20, min(0.0, vol_adjustment))
    
    capital_fraction = base_capital + vol_adjustment + volume_adjustment + mean_reversion_bonus
    capital_fraction = max(0.78, min(1.03, capital_fraction))  # Increased upper bound to 1.03
