...
def strategy(
    ...
    """Volatility & volume-adaptive LP strategy with pool-type-specific capital deployment.

    Key changes from commit 78:
    - Added pool-type-specific capital deployment ceilings
    - Stable pairs get higher capital ceilings (1.25) for tighter ranges
    - Volatile pairs maintain moderate ceilings (1.10)
    - Correlated pairs get medium ceilings (1.15)
    - Slightly tighter rebalance thresholds for stable pairs
    """
    ...
    if step == 0:
        ...
        # Stable pairs: more aggressive rebalancing (1-3 steps)
        # Correlated pairs: moderate rebalancing (2-4 steps)
        # Volatile pairs: conservative rebalancing (2-6 steps)
        if pair_type == "stable":
    ...
    if pair_type == "stable":
        ...
        if fee_signal > 0.04 and recent_vol < 0.015:
            capital_ceiling = 1.30
    elif pair_type == "correlated":
        capital_ceiling = 1.15
        if fee_signal > 0.04 and recent_vol < 0.022:
