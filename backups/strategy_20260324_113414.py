...
def strategy(
    ...
    """Volatility & volume-adaptive LP strategy with mean reversion detection.

    Key changes from commit 15:
    - Added mean reversion signal: when price is close to recent mean, deploy more capital
    - Tighter ranges when mean reversion is detected (price stable relative to volatility)
    - More aggressive capital deployment (base 0.94) when conditions are favorable
    - Asymmetric adjustment: reward stability, penalize large moves
    """
    ...
    base_capital = 0.94  # Slightly more aggressive
    # Squared volatility penalty: more conservative at moderate-to-high vol, more aggressive at low vol
    vol_adjustment = -0.18 * ((recent_vol / 0.05) ** 2)
