...
def strategy(
    ...
    """Volatility & volume-adaptive LP strategy with softer volatility response curve.

    Key changes from commit 69:
    - Increased base capital deployment from 0.97 to 0.975
    - Relaxed upper bound on capital_fraction from 1.0 to 1.02
    - This allows more aggressive deployment when mean reversion and volume signals align
    - Maintains downside protection from volatility dampening via square-root curve
    """
    ...
    if len(prices_so_far) >= 10:
        ...
        if price_std > 0:
            ...
            # Max bonus of 0.03 when deviation < 0.5 std
            mean_reversion_bonus = max(0, 0.03 * (1 - deviation / 0.5))
            mean_reversion_bonus = min(0.03, mean_reversion_bonus)
    ...
    if pair_type == "stable":
        ...
        range_tightness = 1.0 - (mean_reversion_bonus / 0.03) * 0.15  # Up to 15% tighter
    ...
    # --- Volatility & volume-adaptive capital deployment with softer curve ---
    base_capital = 0.975  # Increased from 0.97
    
    # Use square-root dampening for softer volatility response
    # This allows more deployment during moderate vol, protects during spikes
    vol_excess = max(0, recent_vol - 0.02)
    vol_adjustment = -0.20 * np.sqrt(vol_excess / 0.05)  # Softer curve using sqrt
    vol_adjustment = max(-0.19, min(0.0, vol_adjustment))
    
    capital_fraction = base_capital + vol_adjustment + volume_adjustment + mean_reversion_bonus
    capital_fraction = max(0.78, min(1.02, capital_fraction))  # Relaxed upper bound to 1.02
