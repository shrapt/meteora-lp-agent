...
def strategy(
    ...
    """Volatility & volume-adaptive LP strategy with optimized rebalance frequency.

    Key changes from commit 7:
    - Added volume-weighted capital adjustment: deploy more during high-volume periods (better fee capture, lower slippage)
    - Deploy less during low-volume periods (protect against wider spreads)
    - Volume multiplier ranges from 0.8x to 1.2x around the base capital
    - Keeps all successful elements: volatility-adaptive ranges, softer capital adjustment, adaptive rebalance thresholds
    """
    ...
    base_capital = 0.89
    vol_adjustment = -0.20 * (recent_vol / 0.05)  # Softer reduction: -0.20
    capital_fraction = base_capital + vol_adjustment + volume_adjustment
    capital_fraction = max(0.65, min(0.98, capital_fraction))  # Clamp to [0.65, 0.98]
