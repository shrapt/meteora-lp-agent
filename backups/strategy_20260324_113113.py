...
def strategy(
    ...
    """Volatility-adaptive LP strategy with softer capital adjustment and optimized rebalance frequency.

    Key changes:
    - Reduced volatility adjustment coefficient from -0.35 to -0.20 for softer capital reduction during high vol
    - Raised base capital fraction from 87% to 89% to capture more fees
    - Kept the successful high rebalance thresholds (0.75 for stable, 0.70 for volatile)
    - Maintained 3-step minimum rebalance interval
    """
    ...
    # --- Volatility-adaptive capital deployment (softer adjustment) ---
    # Deploy more capital when volatility is low (safer), less when high
    # Softer adjustment to maintain higher fee capture
    base_capital = 0.89
    vol_adjustment = -0.20 * (recent_vol / 0.05)  # Softer reduction: -0.20 instead of -0.35
    capital_fraction = base_capital + vol_adjustment
    capital_fraction = max(0.65, min(0.98, capital_fraction))  # Higher floor at 0.65
