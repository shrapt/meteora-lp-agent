...
def strategy(
    ...
    """Volatility & volume-adaptive LP strategy with volatility-aware rebalance thresholds.

    Key changes from commit 61:
    - Increased base capital deployment from 0.94 to 0.96
    - Reduced volatility penalty factor from -0.25 to -0.15
    - This allows more aggressive deployment while still respecting volatility conditions
    - Tighter bounds on capital_fraction (0.78 to 1.0) to maintain stability
    """
    ...
    base_capital = 0.96  # Increased from 0.94
    vol_adjustment = -0.15 * ((recent_vol - 0.02) / 0.05)  # Reduced penalty from -0.25 to -0.15
    vol_adjustment = max(-0.18, min(0.0, vol_adjustment))
