...
def strategy(
    ...
    Key changes from commit 74:
    - Refined volatility dampening curve for very low volatility periods
    - Allow higher capital deployment (up to 1.05) when recent_vol < 0.025
    - Smoother transition between low and moderate volatility regimes
    - This captures more value during calm market periods
    """
    ...
    # Below 0.025 vol: minimal dampening (allow up to 1.05)
    ...
    if recent_vol < 0.025:
        ...
        # Slight positive adjustment up to +0.075
        vol_adjustment = 0.075 * (1.0 - recent_vol / 0.025)
        vol_adjustment = min(0.075, vol_adjustment)
    ...
    vol_adjustment = max(-0.20, min(0.075, vol_adjustment))
    
    capital_fraction = base_capital + vol_adjustment + volume_adjustment + mean_reversion_bonus
    capital_fraction = max(0.78, min(1.05, capital_fraction))  # Increased upper bound to 1.05
