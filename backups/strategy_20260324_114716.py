...
def strategy(
    ...
    Key changes from commit 36:
    - Increased mean reversion bonus cap to 0.03 (was 0.02) to reward tighter ranges when price is near mean
    - Raised capital floor to 0.76 (was 0.74) to allow more deployment in favorable conditions
    - These changes encourage more aggressive positioning in mean-reverting, low-volatility regimes
    """
    ...
    if len(prices_so_far) >= 10:
        ...
        if price_std > 0:
            ...
            # Max bonus of 0.03 when deviation < 0.5 std (increased from 0.02)
    ...
    if len(volumes_so_far) >= 5:
        ...
        if avg_volume > 0:
            ...
            # Capped to ±0.18 to improve responsiveness to volume changes
    ...
    if step == 0:
        ...
        # Rebalance threshold: trigger when price drifts beyond 45% of range width
        drift_threshold = range_width * 0.45
    ...
    base_capital = 0.94
    # More aggressive volatility adjustment: -0.25 per 0.05 volatility
    # This penalizes deployment more heavily in volatile conditions
    vol_adjustment = -0.25 * ((recent_vol - 0.02) / 0.05)
    vol_adjustment = max(-0.20, min(0.0, vol_adjustment))  # Cap between -0.20 and 0
    capital_fraction = base_capital + vol_adjustment + volume_adjustment + mean_reversion_bonus
    capital_fraction = max(0.76, min(1.0, capital_fraction))  # Clamp to [0.76, 1.0] (raised from 0.74)
