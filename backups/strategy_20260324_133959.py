...
def strategy(
    ...
    """Volatility & volume-adaptive LP strategy with range-center proximity bonus.

    Key changes from commit 78:
    - Added price proximity to range center bonus: +0.03 when centered, -0.05 near edges
    - This reduces capital deployment when rebalancing is likely (price near boundaries)
    - Improves net yield by reducing slippage costs from frequent rebalancing
    """
    ...
    # --- Price proximity to range center bonus ---
    # Reduce capital when price is near edges (high rebalance risk)
    # Increase capital when price is safely centered
    ...
    if range_width > 0:
        ...
        # When centered (offset < 0.4): +0.03 bonus
        # When at edge (offset > 0.7): -0.05 penalty
        if price_offset < 0.4:
            proximity_bonus = 0.03 * (1.0 - price_offset / 0.4)
        elif price_offset > 0.7:
            proximity_bonus = -0.05 * ((price_offset - 0.7) / 0.3)
        else:
            proximity_bonus = 0.03 * (1.0 - (price_offset - 0.4) / 0.3)
        proximity_bonus = max(-0.05, min(0.03, proximity_bonus))
    else:
        proximity_bonus = 0.0
    ...
    capital_fraction = base_capital + vol_adjustment + volume_adjustment + mean_reversion_bonus + volume_momentum + proximity_bonus
    capital_fraction = max(0.78, min(1.10, capital_fraction))  # Increased upper bound to 1.10
