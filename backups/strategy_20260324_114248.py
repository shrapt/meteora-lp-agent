...
def strategy(
    ...
    Key changes from commit 34:
    - More aggressive volatility penalty: -0.25 per 0.05 vol (was -0.20) to reduce over-deployment in volatile markets
    - Refined mean reversion bonus cap to 0.02 (was 0.04) for more conservative deployment
    - Adjusted capital floor to 0.74 (was 0.76) to allow slightly lower deployment in extreme vol
    - Increased volume adjustment cap to ±0.18 (was ±0.15) for better responsiveness to volume spikes
    """
    ...
    if len(prices_so_far) >= 10:
        ...
        if price_std > 0:
            ...
            # Max bonus of 0.02 when deviation < 0.5 std (reduced from 0.04)
    ...
    if pair_type == "stable":
        ...
        range_tightness = 1.0 - (mean_reversion_bonus / 0.02) * 0.15  # Up to 15% tighter
    ...
    base_capital = 0.94
    # More aggressive volatility adjustment: -0.25 per 0.05 volatility (was -0.20)
    ...
    capital_fraction = max(0.74, min(1.0, capital_fraction))  # Clamp to [0.74, 1.0]
