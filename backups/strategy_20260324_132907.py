...
def strategy(
    ...
    - Added volume-momentum signal: increases capital deployment when volume is rising
    - This captures better fee opportunities during high-activity periods
    - Reduces capital deployment when volume is falling (lower fee capture potential)
    - Volume momentum is smoothed over 10 steps to avoid noise
    """
    ...
    volume_momentum = 0.0  # -0.08 to +0.08 adjustment
    if len(volumes_so_far) >= 10:
        ...
        if prior_vol_avg > 0:
            ...
            # Max adjustment: ±0.08 for 2x volume change
            momentum_signal = np.log(vol_momentum_ratio + 0.1) / np.log(2.0)
            volume_momentum = 0.08 * momentum_signal
            volume_momentum = max(-0.08, min(0.08, volume_momentum))
    ...
    if step == 0:
        ...
        # Minimum interval: don't rebalance more than once every 5 steps (increased from 4)
